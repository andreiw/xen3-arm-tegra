...
#define CONFIG_CPU_XSCALE

#ifdef CONFIG_CPU_XSCALE
#define DIV_UP(x,y) __divdi3((x) + (y) - 1, y)
#else
...
#ifdef CONFIG_CPU_XSCALE		  
        inf->score[EXTRA_PEN_Q] = __divdi3((inf->period << 10), inf->short_block_lost_tot);
#else		  
...
#ifdef CONFIG_CPU_XSCALE		  
				inf->score[EXTRA_UTIL_Q] = __divdi3(inf->period << 10, inf->slice);
#else		  
...
#ifdef CONFIG_CPU_XSCALE		  
            inf->score[EXTRA_UTIL_Q] = __divdi3((1<<17) ,inf->extraweight);
#else		  
    ...
    if ( !list_empty(runq) )
    {
        ...
        if ( !list_empty(waitq) )
        {
            ...
            /*rerun scheduler, when scheduled domain reaches it's
              ...
              gets ready*/
            ret.time = MIN(runinf->slice - runinf->cputime, PERIOD_BEGIN(waitinf) - now);
				/* Bokdeuk: below code causes system abort */
    ...
    struct sedf_vcpu_info* inf, s_time_t now)
{
    ...
    if ( likely(inf->block_abs) )
    {
        ...
        {
#ifdef CONFIG_CPU_XSCALE		  
            inf->score[0] = __divdi3((inf->period << 10), inf->short_block_lost_tot);
#else				
    ...
    else
    {
        ...
        if ( likely(inf->unblock_abs) )
        {
            ...
            /*scale slice accordingly, so that utilisation
              stays the same*/
#ifdef CONFIG_CPU_XSCALE				
            inf->slice = __divdi3(inf->period * inf->slice_orig, inf->period_orig);
#else				
            inf->slice = (inf->period * inf->slice_orig)
                / inf->period_orig;
#endif				
...
#ifdef CONFIG_CPU_XSCALE
                sumt[p->processor] += __divdi3((WEIGHT_PERIOD * EDOM_INFO(p)->slice_orig),
