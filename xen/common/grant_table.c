...
static inline int get_maptrack_handle(grant_table_t *t)
{
    unsigned int h;
...
static inline void put_maptrack_handle(grant_table_t *t, int handle)
{
 ...
 */
static void __gnttab_map_grant_ref(struct gnttab_map_grant_ref *op)
{
    struct domain *ld, *rd;
    struct vcpu   *led;
    int            handle;
    ...
    if ( unlikely((handle = get_maptrack_handle(ld->grant_table)) == -1) )
    {
        ...
        new_mt = pages_m_alloc(lgt->maptrack_order + 1);
        if ( new_mt == NULL )
        {
            put_domain(rd);
        ...
        pages_m_free(lgt->maptrack, lgt->maptrack_order);
        lgt->maptrack          = new_mt;
    ...
    if ( op->flags & GNTMAP_device_map )
        act->pin += (op->flags & GNTMAP_readonly) ? GNTPIN_devr_inc : GNTPIN_devw_inc;

    if ( op->flags & GNTMAP_host_map )
        act->pin += (op->flags & GNTMAP_readonly) ? GNTPIN_hstr_inc : GNTPIN_hstw_inc;
    ...
    ld->grant_table->maptrack[handle].ref_and_flags = (op->ref << MAPTRACK_REF_SHIFT) | (op->flags & MAPTRACK_GNTMAP_MASK);
...
static long gnttab_map_grant_ref(GUEST_HANDLE(gnttab_map_grant_ref_t) uop, unsigned int count)
{
    int i;
...
static void __gnttab_unmap_grant_ref(struct gnttab_unmap_grant_ref *op)
{
    domid_t          dom;
    grant_ref_t      ref;
    struct domain   *ld, *rd;
...
static long gnttab_unmap_grant_ref(GUEST_HANDLE(gnttab_unmap_grant_ref_t) uop, unsigned int count)
{
    int i;
...
static long gnttab_setup_table(GUEST_HANDLE(gnttab_setup_table_t) uop, unsigned int count)
{
 ...
 */
static int gnttab_prepare_for_transfer(struct domain *rd, struct domain *ld, grant_ref_t ref)
{
    grant_table_t *rgt;
    grant_entry_t *sha;
    domid_t        sdom;
...
static long gnttab_transfer(GUEST_HANDLE(gnttab_transfer_t) uop, unsigned int count)
{
...
long do_grant_table_op(unsigned int cmd, GUEST_HANDLE(void) uop, unsigned int count)
{
    long rc;
    struct domain *d = current->domain;
...
int grant_table_create(struct domain *d)
{
    ...
    if ( (t->maptrack = pages_m_alloc1()) == NULL )
        goto no_mem;
    t->maptrack_order = 0;
    ...
    /* Shared grant table. */
    t->shared = pages_m_alloc(ORDER_GRANT_FRAMES);
    if ( t->shared == NULL )
        goto no_mem;
...
void grant_table_destroy(struct domain *d)
{
    ...
    pages_m_free(t->shared, ORDER_GRANT_FRAMES);
    pages_m_free1(t->maptrack);
    xfree(t->active);
    xfree(t);
